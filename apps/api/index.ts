...
import {
	googleCalendarDateToUtcIso,
	mobilizeTimestampToUtcIso,
} from "@connect-utah-today/utils/toUtcIsoString";
import {
...
async function fetchGoogleCalendarEvents(
	...
	if (!apiKey) {
		console.log(
			"Google Calendar API key not set. Skipping Google Calendar events fetch.",
		);
		return { data: [] };
	}
	...
	if (!calendarId) {
		console.log(
			"Google Calendar ID not set. Skipping Google Calendar events fetch.",
		);
		return { data: [] };
	}
	...
	try {
		const response: AxiosResponse<GoogleCalendarApiEventsResponse> =
			await axios.get(url);
...
async function fetchMobilizeEvents(
	reqQuery: EventsApiQueryParams,
): Promise<MobilizeEventsApiResponse> {
	...
	if (!activeFilters.organizations && (reqQuery as any).organizations) {
		try {
			activeFilters.organizations =
				typeof (reqQuery as any).organizations === "string"
	...
	try {
		while (nextUrl) {
			...
			const response: AxiosResponse<MobilizeApiEventsResponse> =
...
app.get(
	"/mobilize-events",
	async (req: Request<{}, {}, {}, EventsApiQueryParams>, res: Response) => {
		try {
			...
			if (error instanceof Error) {
				res.status(500).json({
					error: "Failed to fetch Mobilize events",
...
app.get(
	...
	async (req: Request<{}, {}, {}, EventsApiQueryParams>, res: Response) => {
		try {
			...
			if (error instanceof Error) {
				res.status(500).json({
					error: "Failed to fetch Mobilize organizations",
					details: error.message,
				});
			}
		}
