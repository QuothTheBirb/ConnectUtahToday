...
const useDisclaimerContext = () => {
	...
	if (!context) {
		throw new Error(
			"useDisclaimerContext must be used within a DisclaimerProvider",
		);
	}
...
export const DisclaimerPopup = ({
	...
	const footer = (
		<button className={styles.closeDisclaimer} onClick={onClose}>
			Close
		</button>
	);
	...
	return (
		<Popup
			...
			footer={footer}
		>
			<div className={styles.disclaimerContent}>{children}</div>
		</Popup>
	);
};
